...
%DIF DEL user_guide.tex   Wed Jun  2 15:41:42 2021
%DIF ADD lls.tex          Wed Jun  2 15:35:50 2021
...
\usepackage[expert,altbullet]{lucidabr} %DIF > 
 %DIF > 
\usepackage[colorlinks=true, %DIF > 
            linkcolor=blue, %DIF > 
            urlcolor=blue, %DIF > 
            citecolor=blue, %DIF > 
            linkbordercolor={0.8 0.8 0.8}, %DIF > 
            menubordercolor={0 1 0}]{hyperref} %DIF > 
%DIF -------
...
\author{H.~E.~Motteler and L.~L.~Strow \\
  \\
  \DIFdelbegin \DIFdel{UMBC Atmospheric Spectroscopy Lab }\DIFdelend \DIFaddbegin \DIFadd{University of Maryland Baltimore County (UMBC)}\DIFaddend \\
  Joint Center for Earth Systems Technology \DIFaddbegin \\
  \DIFadd{Atmospheric Spectroscopy Lab }\DIFaddend \\
}
\date{\today}
...
ascending 1:30 PM (13:30) equatorial crossing time and are combined \DIFaddbegin \DIFadd{as
shown in \ref{fig0} }\DIFaddend to make the CHIRP-1330 product.  Although the primary
product is a single continuous record, support products in the form of
translations for other NOAA sounders are also provided as CHIRP-CAL
products.  \DIFaddbegin \DIFadd{Using these support product users can create radiance time
series that cross over from one instrument to the next at different times
than the }\DIFaddend CHIRP-1330 \DIFdelbegin \DIFdel{is made available
by JPL Sounder SIPS as a product at the GSFC-DISC}\DIFdelend \DIFaddbegin \DIFadd{product, as long as the CHIRP-CAL product exists for
the desired times}\DIFaddend .
...
The organization of other support variables \DIFaddbegin \DIFadd{is }\DIFaddend generally similar, and
...
\DIFdelbegin \DIFdel{geometery }\DIFdelend \DIFaddbegin \DIFadd{geometry }\DIFaddend and observation time.  See section \ref{sampling} for
...
\begin{figure} % source plot_tbin.m
  ...
  \caption{CrIS scan \DIFdelbeginFL \DIFdelFL{geometery}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{geometry}\DIFaddendFL , from the NOAA User's Guide \cite{ntech1}}
  \label{fig7}
\end{figure}
...
The AIRS L1c scan \DIFdelbegin \DIFdel{geometery }\DIFdelend \DIFaddbegin \DIFadd{geometry }\DIFaddend is organized as 90 cross-track by 135
along-track observations, giving $90 \times 135 = 12150$ obs per
granule.  The CrIS L1b scan \DIFdelbegin \DIFdel{geometery }\DIFdelend \DIFaddbegin \DIFadd{geometry }\DIFaddend is organized as 9 FOVs arranged
...
\section{\DIFadd{Getting Help}}
\DIFadd{Further information and help on using CHIRP can be found at
}\href{https://infraredclimate.org}{https://infraredclimate.org}\DIFadd{, or send
email to }\href{mailto:chirp@infraredclimate.org}{chirp@infraredclimate.org}\DIFadd{.
This web site will also contain information on the Level 1b products
produced at GES-DIS for the series of CrIS instruments, starting with
SNPP-CrIS and continued with the JPSS series of CrIS instruments.
}\DIFaddend 
...
The 12 fields correpsond to CHIRP attributes, and are defined in the
